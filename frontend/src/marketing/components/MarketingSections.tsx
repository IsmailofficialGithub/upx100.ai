  ...
  return (
    <section className="py-16 md:py-20">
      <div className="container-main">
        <p className="font-mono text-[11px] uppercase tracking-[0.25em] text-[hsl(var(--muted-foreground))] text-center mb-3 mkt-fade-up">Two Paths. One Platform.</p>
        ...
        <p className="text-sm text-[hsl(var(--muted-foreground))] text-center max-w-lg mx-auto mb-9 leading-relaxed mkt-fade-up">
          ...
          {(['defend', 'attack'] as const).map((key, i) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-[13px] text-[hsl(var(--muted-foreground))] leading-relaxed mb-5">
                  {isDefend
                ...
                <div className="font-mono text-2xl font-bold tracking-tight mb-1">
                  {card.price}<span className="text-[13px] text-[hsl(var(--muted-foreground))] font-normal">{card.period}</span>
                </div>
                <p className="font-mono text-[10px] text-[hsl(var(--muted-foreground))] mb-5">{card.setup} · {card.minutes}</p>
                <ul className="space-y-2 list-none p-0 m-0">
                  {card.features.map((f) => (
                    <li key={f} className="text-xs text-[hsl(var(--muted-foreground))] flex gap-2 py-1.5 border-b border-up-dark-3 last:border-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {points.map((p, i) => (
            ...
            >
              ...
              {p.body && <p className="text-[13px] text-[hsl(var(--muted-foreground))] leading-relaxed">{p.body}</p>}
            </div>
          ))}
        </div>
      </div>
...
export function DemoCallForm({ id = 'demo' }: { id?: string }) {
  ...
  return (
    <div id={id} className="bg-up-dark-1 border border-up-dark-4 rounded-xl p-8 md:p-9">
      <label htmlFor={`name-${id}`} className="font-mono text-[10px] uppercase tracking-widest text-[hsl(var(--muted-foreground))] block mb-2">
        Your Name
      </label>
      <input
        id={`name-${id}`}
        ...
        autoComplete="name"
        className="w-full px-4 py-3.5 bg-up-dark-2 border border-up-dark-4 rounded-md text-[hsl(var(--foreground))] font-mono text-sm outline-none focus:border-up-green mb-4"
      />
      <label htmlFor={`phone-${id}`} className="font-mono text-[10px] uppercase tracking-widest text-[hsl(var(--muted-foreground))] block mb-2">
        Your Phone Number
      </label>
      <input
        id={`phone-${id}`}
      ...
      <div className="flex gap-3 items-start mb-5">
        ...
        />
        <label htmlFor={`consent-${id}`} className="text-[11px] text-[hsl(var(--muted-foreground))] leading-relaxed">
      ...
      <button
        ...
        className={`w-full py-3.5 rounded-lg font-display font-bold text-sm transition-all ${
          consent && !isSubmitting
            ? 'bg-up-green text-up-on-green cursor-pointer hover:shadow-[0_0_32px_rgba(0,255,136,0.15)]'
            : 'bg-up-dark-3 text-[hsl(var(--muted-foreground))] opacity-30 cursor-not-allowed'
        }`}
      >
        {isSubmitting ? 'Starting call…' : 'Call Me Now →'}
      </button>
      <p className="font-mono text-[10px] text-[hsl(var(--muted-foreground))] text-center mt-3">Takes ~15 seconds. No obligation. No follow-up spam.</p>
    </div>
  );
}
    ...
    <div className="bg-up-dark-1 border border-up-dark-4 rounded-xl p-8 md:p-9 relative">
      ...
      <div className="flex gap-2.5 mb-5">
        {(['defend', 'attack'] as const).map((v) => (
          <label key={v} className="flex-1 flex items-center gap-2 px-3.5 py-3 bg-up-dark-2 border border-up-dark-4 rounded-md cursor-pointer font-mono text-[11px] text-[hsl(var(--muted-foreground))]">
